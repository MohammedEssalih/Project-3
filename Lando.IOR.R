...
Lando_IOR <- function(a,n,m,p) {
  ...
  return(data.frame(a=a,IOR_power = v1, DOR_power = v2))
}
...
for (m in m_values) {
  temp_data <- do.call(rbind, lapply(x, function(i) Lando_IOR(i, n, m, p)))
...
write.csv(results, "Finall.IOR_200.csv")
