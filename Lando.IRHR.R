...
S <- function(i, m) {
  sum(1 / seq(i , m))  # Harmonic series for the order statistics
}
...
Lando_DRHR <- function(a,n,m,p) {
  
  # True "pi" based on exponential distribution
  TR <- sapply(1:m, function(i)  exp(-S(i, m)))
  ...
  # 50000ulate samples from Exp and compute statistics
  for (ii in 1:5000) {
  ...
  return(data.frame(a=a,IRHR_power = v1, DRHR_power = v2))
}

Lando_DRHR(1.5, 25, 1, 1)
...
for (m in m_values) {
  temp_data <- do.call(rbind, lapply(x, function(i) Lando_DRHR(i, n, m, p)))
...
# Convert columns
colnames(results) <- c("a", "IRHR_power", "DRHR_power", "source")

# Plot the power of convexity and concavity
ggplot(results, aes(x = a, y = DRHR_power, shape = factor(source))) +
  geom_line() +
  geom_point() +
  labs(title = "Power of DRHR Test for n=25",
       x = "Shape parameter 'a'",
       y = "Power") +
  theme_minimal() +
  ylim(0, 1) +
...
write.csv(results, "Finall.DRHR_25.csv")
